...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Verify subscription request:', { subscriptionId, paymentId, signature, fundId })

    // For subscriptions, we verify the subscription signature
    // Note: Razorpay subscription verification uses subscription_id + payment_id
    if (!subscriptionId || !signature) {
      return NextResponse.json({ 
        error: 'Missing required fields: subscriptionId and signature' 
      }, { status: 400 })
    }

    // Verify Razorpay signature
    // For subscriptions, signature = hmac_sha256(payment_id, secret)
    const isValidSignature = verifySubscription(subscriptionId, paymentId || '', signature)
