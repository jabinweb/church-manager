...
export function ChatLayout() {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        // Check for pending conversation BEFORE fetching
        const pendingId = sessionStorage.getItem('openConversation')
        console.log('Pending conversation ID from sessionStorage:', pendingId)
        ...
        if (conversationsResponse.ok) {
          ...
          console.log('Initial conversations fetched:', data.conversations?.length)
          const fetchedConversations = data.conversations || []
          setConversations(fetchedConversations)
          
          // Handle pending conversation selection after fetching
          if (pendingId) {
            console.log('Looking for pending conversation:', pendingId)
            const conversation = fetchedConversations.find((c: UnifiedConversation) => c.id === pendingId)
            if (conversation) {
              ...
              console.log('Conversation not found in fetched list')
            }
