...
export function ChatArea({
  ...
  // Get the other participant for calls
  const otherParticipant = selectedConversation?.participants?.find(
    p => p.userId !== session?.user?.id
  )

  const handleStartCall = (type: 'audio' | 'video') => {
    if (!otherParticipant || !otherUserOnline) {
  ...
  useEffect(() => {
    ...
    if (!otherParticipant) {
      console.log('ChatArea: No other participant found')
      return
    }

    console.log('ChatArea: Checking online status for user:', otherParticipant.userId)

    const checkOnlineStatus = async () => {
      try {
        const response = await fetch(`/api/users/online?userId=${otherParticipant.userId}`)
        if (response.ok) {
          const data = await response.json()
          console.log('ChatArea: Online status response:', data)
    ...
    // Check again after 2 seconds (in case SSE connection is still establishing)
    const initialTimeout = setTimeout(checkOnlineStatus, 2000)

    // Poll every 10 seconds for more responsive status
            ...
            {selectedConversation.type === 'DIRECT' && (
              <>
                <Button 
                  ...
                  disabled={!otherUserOnline || callState.status !== 'idle'}
                  title={otherUserOnline ? 'Start audio call' : 'User is offline'}
                >
                ...
                <Button 
                  ...
                  disabled={!otherUserOnline || callState.status !== 'idle'}
                  title={otherUserOnline ? 'Start video call' : 'User is offline'}
                >
